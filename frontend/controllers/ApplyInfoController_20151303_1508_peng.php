...
//用foreach从第二行开始写数据，因为第一行是表头
        ...
        foreach($model as $val){
            $phpexcel->getActiveSheet() ->setCellValue('A'.$i,$val['name']['username'])
...
//设置header
    ...
    public function actionHuojiang(){
        ...
        $connection = Yii::$app->db;
        $username = "update bm_apply_info set position=$position WHERE user_id=$user_id AND event_id=$event_id";
