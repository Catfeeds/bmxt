...
//            $event_ids="select event_id from bm_shoucang WHERE user_id=$user_id";
//            $data2 = $connection->createCommand($event_ids);
//            $event_idl = $data2->queryAll();
//            foreach($event_idl as $v){
//                $event_id=($v['event_id']);
//                $sql1 = "SELECT COUNT(*) FROM bm_relese_event INNER JOIN bm_apply_info ON bm_relese_event.id=bm_apply_info.event_id WHERE bm_apply_info.event_id= $event_id";
//                $data1 = $connection->createCommand($sql1);
//                $model2 = $data1->queryOne();
//
//            }
            $user_id = Yii::$app->user->id;
            $event_ids = Shoucang::find()->where(['user_id'=>$user_id])->all();
            $bm = ApplyInfo::find()->where(['user_id'=>$user_id])->all();

            if(!empty($bm)){
                foreach($bm as $key=>$v){
                    $event_id1 = $v->event_id;

                }
                foreach($event_ids as $key=>$v) {
                    $event_id = $v->event_id;
                }
                $model1 = relese::find()->joinWith('orders')->where(['bm_relese_event.id' =>$event_id])->all();
                $saishi = ReleseEvent::find()->where(['id'=>$event_id1])->all();
                $tuijian=ReleseEvent::find()->orderBy('id desc')->all();
                ...
                $model1 = relese::find()->joinWith('orders')->where(['bm_relese_event.id' =>$event_id])->all();
                $tuijian=ReleseEvent::find()->orderBy('id desc')->all();
                return $this->render('baomingyonghu',['tuijian'=>$tuijian,'model1'=>$model1,]);
            }else{
                $tuijian=ReleseEvent::find()->orderBy('id desc')->all();
                return $this->render('baomingyonghu',['tuijian'=>$tuijian]);
            }
        }
    ...
    public function actionSerch()
    {
        ...
        $price=Yii::$app->request->get()['prize'];
        $orgnize=Yii::$app->request->get()['orgnize'];
        $type=Yii::$app->request->get()['type'];
        $place=Yii::$app->request->get()['place'];
        // echo($price);

        // $apply_money=Yii::$app->request->get()['apply_money'];
        $data = relese::find()->where(['apply_money'=>$price]);
        $pages = new Pagination(['totalCount' =>$data->count(), 'pageSize' => '6']);
        $model = $data->offset($pages->offset)->limit($pages->limit)->all();
