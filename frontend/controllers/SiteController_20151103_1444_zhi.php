...
class SiteController extends Controller
{
    ...
    public function actionDuiyou()
    {
        $id=$_GET['id'];
        $users=new Users();
        $model=Users::find()->where(['team_id'=>$id]);
        $dataProvider = new ActiveDataProvider([
            //'query' => ApplyInfo::find($event_id,$user_id),
            'query'=>$users->find()->where(['team_id'=>$id])
        ]);
    ...
    public function actionYonghu()
    {
        ...
        if($quanxian==1){
            ...
            $team_id=users::findOne($user_id)->team_id;
            $dataProvider = new ActiveDataProvider([
                'query' => Team::find()->where(['id'=>$team_id]),
            ]);
    ...
    public function actionJiandui(){
        if($_POST)
        {
            ...
            return $this->render('jiandui',['value'=>$value]);
        }else{
    ...
    public function actionJianle(){
        ...
        $connection = Yii::$app->db;
        $gs_id = Yii::$app->user->id;
        $sql="insert into bm_gsteam SET gs_id='$gs_id',gsname='$gsname',gsteam='$gsteam'";
        $data = $connection->createCommand($sql);
        $model = $data->execute();
