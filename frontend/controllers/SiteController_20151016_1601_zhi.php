       ...
       // $num=Yii::$app->request->get()[$model->getNumber()];
        ...
        if($quanxian==1){
            ...
            if(!empty($bm)){
                ...
                foreach($saishi as $key=>$v){
                    $event_type = $v->event_type;
                }
                $bshuzi = ApplyInfo::find()->andWhere(['event_id' =>$event_id1,'user_id'=>$user_id])->count('id');
                $sshuzi = ApplyInfo::find()->andWhere(['event_id' =>$event_id])->count('id');
                ...
                foreach($event_ids as $key=>$v){
                    ...
                    $tuijian=ReleseEvent::find()->orderBy('id desc')->all();
                    $sshuzi = ApplyInfo::find()->andWhere(['event_id' =>$event_id])->count('id');
                    return $this->render('baomingyonghu',['tuijian'=>$tuijian,'model1'=>$model1,'sshuzi' => $sshuzi,]);
