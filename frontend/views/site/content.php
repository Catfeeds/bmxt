...
$this->registerJsFile('js/contents.js');
$this->registerJsFile('js/content_user_infos.js');
...
<div class="brief">
    ...
    <div class="brief_2">
        ...
        <input type="button" name="submit" value="机构信息展示" class="brief_btn" style="margin-left: 5px;" onclick="javascript:location='index.php?r=site/fabu&event_id=<?=$basic_model['id'];?>'">
...
<div class="main">
    <ul class="nav_user" id="nav_user">
        ...
        <li class="nav4_user" style="background: #EEEEEE; color: #FF6701;">赛事动态公告</li>
...
<!--赛事报名-->
    ...
    <div id="sign_comp" style="display: none;">
        <div>
            <form id="get_ajax_info">
                <input type="hidden" id="info-user_id" class="form-control" name="user_id" value="<?=Yii::$app->user->isGuest?0: Yii::$app->user->identity->id;?>">
                <input type="hidden" value= "<?php echo Yii::$app->request->getCsrfToken (); ?>" name= "_csrf" />
                <input type="button" class="btn btn-default" onclick="GetUserInfo()" value="快速报名"/>
            </form>
        </div>
        ...
        <?= $form->field($basic, 'bm_name')->textInput(['maxlength' => true]) ?>
        <?= $form->field($basic, 'sex')->radioList(['1'=>'男','0'=> '女'])?>
        <?= $form->field($basic, 'phone')->textInput() ?>
        <input type="hidden" id="info-bm_time" class="form-control" name="Info[bm_time]" value="<?php echo time();?>">
        <?= $form->field($basic, 'email')->textInput(['maxlength' => true]) ?>
        <?= $form->field($basic, 'id_number')->textInput(['maxlength' => true]) ?>
...
<!--赛事公告-->
    <div id="saishi_gonggao" style="display: none;">
        <?php if(!empty($ssggdd)){foreach($ssggdd as $key=>$val){ ?>
        <p style="text-align: center;font-size: 15pt"><?php echo $val['name'] ?></p>
        <p><?php echo $val['newss'] ?></p>
        <?php }}else{ echo "本赛事暂无公告，谢谢";} ?>
    </div>
<!--以下为友情链接和地图代码-->
