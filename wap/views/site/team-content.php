...
$this->registerJsFile('js/content_user_info.js');
...
<div class="brief">
    ...
    <div class="brief_2">
        <p style="font-weight:700;font-size: 16px; line-height: 18px; margin-top: 10px; width: 350px; height: 36px; overflow: hidden " ><?=$basic_model['event_name'];?></p>
        <p style="width: 350px;height: 20px; overflow: hidden;">赛事地点：<?php
            $a=$basic_model['place'];
            $conf[1] = '济南市';
            $conf[2] = '青岛市';
            $conf[3] = '淄博市';
        ...
        <p>比赛开始时间：<?=$basic_model['begin'];?></p><!-- 小智 -->
        ...
        <input type="button" name="submit" value="查看机构信息" class="brief_btn" style="margin-left: 5px;" onclick="javascript:location='index.php?r=site/fabu&event_id=<?=$basic_model['id'];?>'">
...
<!--赛事报名-->
    ...
    <div id="sign_comp" style="display: none;">
        ...
        <?php $form = ActiveForm::begin(['action' => ['site/team-handle'],'method'=>'post']); ?>
        <?php if(count($team) !== 0){
                for ($i=0; $i < count($team) ; $i++) {
                        echo '<label class="radio-inline"><input name="TeamApplyInfo[leixing]" type="radio" value="'.$i.'"';
                        if($i ==0){echo "checked";}
                        echo '>'.$team[$i]['leixing'].'</label>';
                }
        };
        ?>
        <input type="hidden" id="info-user_id" class="form-control" name="TeamApplyInfo[user_id]" value="<?=Yii::$app->user->isGuest?0: Yii::$app->user->identity->id;?>">
        <input type="hidden" id="info-bm_time" class="form-control" name="TeamApplyInfo[bm_time]" value="<?php echo time();?>">
        <?= $form->field($basic, 'apply_team_id')->textInput(['maxlength' => true]) ?>
        <input type="hidden" id="info-status" class="form-control" name="TeamApplyInfo[bm_status]" value="0">
        <input type="hidden" id="info-project" class="form-control" name="TeamApplyInfo[bm_project]" value="<?php echo Yii::$app->request->get()['id']?>">
        <input type="hidden" name="TeamApplyInfo[event_name]" value="<?=$basic_model['event_name'];?>">
        <input type="hidden" name="TeamApplyInfo[apply_money]" value="<?=$basic_model['apply_money']; ?>">
