...
use frontend\models\CheckIdNum;
...
class SiteController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $newss=Newss::find()->orderBy('id desc')->all();
    ...
    public function actionFasong()
    {
        ...
        if($message){
            ...
            $sql="INSERT INTO bm_message (inviter,tid,beinviter,send_time,team_id) VALUES ('$id', '$tid', '$bid', '$time','$team_id')";
    ...
    public function actionYonghu()
    {
        ...
        if($quanxian==1){
            ...
            $connection = Yii::$app->db;
            $user_id = Yii::$app->user->id;
            $sql="select * from bm_user WHERE sjuser_id=$user_id";
            $data = $connection->createCommand($sql);
            $sjuser_id = $data->queryAll();
            ...
            $sql2="SELECT * FROM bm_apply_info a,bm_relese_event b WHERE a.event_id=b.id AND a.user_id=$user_id AND (a.is_paid=1 OR a.apply_money=0)";
            ...
            //成员列表
            $familyInfoProvider = new ActiveDataProvider([
                'query'=> FamilyInfo::find()->where(['user_id' => $user_id,]),
            ]);
            //团队列表
            $familyListProvider = new ActiveDataProvider([
                'query' => FamilyTeam::find()->where(['user_id' => $user_id]),
            ]);
            return $this->render('baomingyonghu',[
                ...
                'familyInfoProvider' => $familyInfoProvider,
                'familyListProvider' => $familyListProvider,
            ]);
    ...
    public function actionSignup()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $phone_code=$_POST['SignupForm']['phone_code'];
            $phone=$_POST['SignupForm']['phone'];
            if($mobile_code!=$phone_code or $mobile!=$phone){
                return $this->redirect('index.php?r=site/mobile');
            }
    ...
    public function actionContent()
    {
        ...
        $ssggdd = Newss::find()->where(['event_id'=>$event_id])->all();
        ...
        return $this->render('content',['basic_model' => $event_id,
            'field' => $field,
            'basic' => $basic,
            'model1'=>$model1,
            'orders'=>$orders,
    ...
    public function actionHandle(){
        ...
        $cin = new CheckIdNum();
        if(!$cin->checkIdentity($handle['ApplyInfo']['id_number'])){
            Yii::$app->session->setFlash('error', '请输入正确的身份证号！');
            $this->redirect('index.php?r=site/content&id='.$event_id);
        }else{
            //人数判断
                ...
                if($apply_time_now <= $format_time_end){
                    ...
                    if($info['msg'] == 0){
                        ...
                        $this->redirect("index.php?r=site/payment");
                            ...
                            $this->redirect("index.php?r=site/content&id=".$info['id']);
                            Yii::$app->session->setFlash('error', $info['info']);
                    }else{
                            $this->redirect("index.php?r=site/content&id=".$info['id']);
                            Yii::$app->session->setFlash('error', $info['info']);
                    }
        ...
        if(count($pos) == 0){
            ...
            if(isset($pos['respMsg'])){
                ...
                if($type==1){
                    ...
                    TeamApplyInfo::updateAll(['is_paid' => 1],['id' => $id,]);
                }
...
//设置header
         ...
         if($bm<=$people){
            if($apply_time_now <= $format_time_end){
                ...
                if($info['msg'] == 0){
                    ...
                    $session->open();
                    $session->set('info',$info);
                    $this->redirect("index.php?r=site/team-payment");
                }
                else{
                    $this->redirect("index.php?r=site/team-content&id=".$info['id']);
                    Yii::$app->session->setFlash('error', $info['info']);
                }
            }else{
                $this->redirect("index.php?r=site/timeout");
     ...
     * @return
     */
    public function actionTeamPayment(){
        $model = new TeamContent();
        $info = \Yii::$app->session->get('info');
        $apply_id=$info['apply_id'];
        $paid = TeamApplyInfo::find()->andWhere(['id'=>$apply_id])->all();


        return $this->render('team-payment',['model' => '1','info'=> $info,'paid'=>$paid]);
     ...
     * @return string
     */
    public function actionZhanshi()
    {
        $data = Zhanshi::find();
        $pages = new Pagination(['totalCount' =>$data->count(), 'pageSize' => '6']);
        $model = $data->offset($pages->offset)->limit($pages->limit)->all();

        return $this->render('zhanshi',[
            'model' => $model,
            'pages' => $pages,
        ]);
    ...
    public function actionXiaji(){
        $user_id = $_GET['id'];
        $connection = Yii::$app->db;
        $sql="select * from bm_relese_event where user_id=$user_id";
        $data = $connection->createCommand($sql);
        $grss = $data->queryAll();
        $sql1="select * from bm_relese_team_event where user_id=$user_id";
        $data1 = $connection->createCommand($sql1);
        $ttss = $data1->queryAll();
        return $this->render('xiaji',[
            'grss'=>$grss,
            'ttss'=>$ttss
        ]);
    }
